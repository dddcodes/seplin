...
function removeConfirm(ubication){
    ...
    //aparece el btn para confirmar en la btnBar
    ...
    const condition = element.default;
    ...
    if(questsToRemove.includes(ubication)){
        ...
        box.style.background = (condition)?"var(--llamative-clr)":"var(--main-clr-2)";
        ...
        box.style.background = (condition)?"var(--llamative-clr-darker)":"var(--main-clr-3)";
...
function generateQuizes(){
    ...
    for (let i = catalog_quizesGenerated; m < catalog_questsPerClick; i++) {
        ...
        <br/>
        <div class="simpleBox ${color}" asignature="${element.cat1}" id="quest${i}" style="text-align: left; ">
            

            <p style="color: var(--main-clr);" style="margin: 10px;">${isDefault}</p>
        
            
            <button class="miniBtn fail" id="deleteBtnQuest${i}" onclick="removeConfirm(${i})">
...
function view(ubication, specialClass = false){

    const item = document.querySelector(`#quest${ubication}viewBox`);
    const showBtn = document.querySelector(`#catalogQuestViewBtn${ubication}`);
    const removeBtn = document.querySelector(`#catalogQuestRemoveViewBtn${ubication}`);

    const e = questList[ubication];

    appear(item);

    const classes = (specialClass)?
        "niceBox mainClr3 subYellowBox":
        "niceBox mainClr3";
    const styles = "border-radius: 0;";
    const content = `
    <hr/><br/>
    <p class="${classes}" style="${styles}">
